<template>
            ...
            <v-flex md12>
                ...
                >
                    <v-tooltip bottom>
                        ...
                        <span><font color="white">Add New GL Code</font></span>
                    </v-tooltip>
                    {{pagination}}
                    <v-data-table
                        :headers="headers"
                        ...
                        <template slot="items" slot-scope="{ item }">
                            ...
                            <td align="center">
                                <v-tooltip top content-class="top">
                                    ...
                                    <span><font color="white">Edit GL Code Information</font></span>
                                </v-tooltip>
                                <v-tooltip top content-class="top">
                                    <v-btn
                                        slot="activator"
                                    ...
                                    <span><font color="white">Remove GL Code</font></span>
                                </v-tooltip>
                            </td>
                        </template>
                    </v-data-table>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
          <span class="headline">GL Code</span>
        </v-card-title>
        <v-card-text>
         <v-form
            ref="form"
...
export default {
    ...
    methods: {
        ...
        async update() {
            try {
                this.loading = true
                const response = await this.$axios.put(`/api/v1/gls/${this.gl.id}`, this.gl)
                this.fetch()
                this.dialog = false
                this.loading = false
